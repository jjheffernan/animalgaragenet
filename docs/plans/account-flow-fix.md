...
**Production symptom:** Header shows “Sign In” on https://animalgarage.netlify.app even after users attempt magic link / OAuth. `/account` and `/loyalty` gate correctly but session never persists.
...
2. **`PUBLIC_SITE_URL` mismatch** — Magic link and OAuth `redirectTo` / `emailRedirectTo` are built from `config.siteUrl` (`PUBLIC_SITE_URL`), not `event.url.origin`. If set to `https://animalgarage.net` while browsing `animalgarage.netlify.app`, email links and OAuth callbacks land on the wrong host → session cookies never set on the browsed domain.
3. **Supabase redirect URL allowlist** — Callback `https://animalgarage.netlify.app/auth/callback` not added in Supabase dashboard → `exchangeOAuthCode` / magic link verify fails.
...
| `PUBLIC_SUPABASE_ANON_KEY` | `eyJ...` | Publishable key — safe in client |
| `SUPABASE_SERVICE_ROLE_KEY` | `eyJ...` | **Server only** — scopes: all |
| `PUBLIC_SITE_URL` | `https://animalgarage.netlify.app` | **Must match browser URL** until custom domain cutover |

**Do not set:** `DEV_ADMIN`, `LOCAL_DEV_AUTH`

**At custom domain cutover:** Change `PUBLIC_SITE_URL` to `https://animalgarage.net` and redeploy.
...
| Setting | Value |
|---------|-------|
| Site URL | `https://animalgarage.netlify.app` (update to `https://animalgarage.net` at cutover) |
| Redirect URLs | Add **both**: |
| | `https://animalgarage.netlify.app/auth/callback` |
...
**No custom cookie domain is set** — cookies bind to the host that served the response. This is correct for `animalgarage.netlify.app` as long as:
...
**Verify:** After sign-in, DevTools → Application → Cookies should show `sb-<ref>-auth-token` (or chunked variants) on `animalgarage.netlify.app`.
...
| Change | Why |
|--------|-----|
| Add `animalgarage.netlify.app` and `*.netlify.app` to `isProductionHostname()` | Prevent accidental `DEV_ADMIN` bypass on Netlify |
...
1. Open https://animalgarage.netlify.app/auth/sign-in in a clean browser profile.
2. Enter email → submit → check inbox.
3. Click link → should land on `https://animalgarage.netlify.app/auth/callback?code=...` (not `animalgarage.net` unless that is `PUBLIC_SITE_URL`).
...
- [ ] Authenticated session visible in header on `animalgarage.netlify.app`

...
<p class="mt-1 text-sm text-zinc-400">Save a draft anytime. Submit when ready for admin approval.</p>
...
{#if form?.success}
	<div class="mt-6 rounded-sm border border-emerald-900/50 bg-emerald-950/30 px-4 py-4">
		...
		<a href={resolve('/account/builds')} class="mt-3 inline-block text-sm text-red-500 hover:text-red-400">← My build logs</a>
	</div>
{:else}
	<form
		method="POST"
	...
	>
		...
		<div class="flex flex-wrap gap-3">
			<button type="submit" formaction="?/saveDraft" disabled={saving} class="rounded-sm border border-zinc-700 px-4 py-2 text-sm font-bold uppercase tracking-wider text-zinc-300 transition hover:border-red-600 hover:text-white disabled:opacity-50">
				Save draft
			</button>

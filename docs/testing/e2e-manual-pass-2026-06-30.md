...
| -------- | ----- | ------ |
| `PUBLIC_SUPABASE_URL` | *(empty)* | Mock `ag-session` cookie auth via `hooks.server.ts` |
| `PUBLIC_SUPABASE_ANON_KEY` | *(empty)* | Same |
| `SUPABASE_SERVICE_ROLE_KEY` | *(empty)* | Build logs + testimonials use in-memory mock stores |
| `PUBLIC_SALEOR_API_URL` | *(empty)* | Mock catalog + localStorage cart |
...
| AUTH-2 | P0 (fixed this pass) | `src/routes/auth/sign-in/+page.server.ts` | Mixed `default` + `devSignIn` named actions caused 500: *"When using named actions, the default action cannot be used"* — renamed to `magicLink` + `action="?/magicLink"` |

		...
		<div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each data.categories as cat (cat.id)}
				<a
					...
					class="group overflow-hidden rounded-sm border border-zinc-800 bg-zinc-900 transition hover:border-red-600/50"
				>
					{#if cat.imageUrl}
						<img
							src={cat.imageUrl}
							alt={cat.name}
							class="aspect-[16/9] w-full object-cover transition group-hover:scale-105"
							loading="lazy"
						/>
					{/if}
					<div class="p-4">

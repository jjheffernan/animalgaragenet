...
>
	<section class="border-b border-zinc-800 bg-zinc-900/50 py-16">
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			...
			<h1 class="mt-2 font-display text-4xl font-bold uppercase tracking-wider text-white">
				Garage Squad
			</h1>
				...
				{#if nextLevel}
					<div class="mt-6">
						...
						<p class="mt-2 text-xs text-zinc-500">
							{nextLevel.xpRequired - garageXp.xp} XP to {nextLevel.title}
						</p>
					</div>
				{/if}
			</div>
		...
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			...
			<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
				{#each garageLevels as level (level.level)}
					<div
						class="rounded-sm border p-6 {garageXp.xp >= level.xpRequired
		...
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			...
			{:else}
				<p class="mt-8 text-center text-sm text-zinc-500">
					Be the first to share your Garage Squad story.
				</p>
			{/if}
			...
			<div class="mx-auto mt-12 max-w-xl rounded-sm border border-zinc-800 bg-zinc-900/50 p-8">
				...
				<p class="mt-2 text-sm text-zinc-400">
					Tell the squad about your build, your experience, and what loyalty perks mean to you.
					Approved reviews earn +50 XP.
				</p>

				{#if form?.success}
					<p
						class="mt-4 rounded-sm border border-red-900/50 bg-red-950/20 px-4 py-3 text-sm text-red-400"
					>
						{form.message}
					</p>
				{/if}
				...
				{#if hasPendingReview}
					<p class="mt-4 text-sm text-zinc-500">
						Your review is awaiting moderation — we'll notify you when it's live.
					</p>
				{:else}

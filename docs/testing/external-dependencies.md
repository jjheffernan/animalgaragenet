...
| ----------------- | ------------------- | ---------------------------------------------------- | ------------------------------------------------- | --------------------- | --------------------------------------------------------- | --------------------------------------------- | ----------------------------------- | ------------------------------------ |
| `supabase-auth`   | Supabase Auth       | `src/lib/server/supabase/auth.ts`, `hooks.server.ts` | `SUPABASE_DATABASE_URL`, `SUPABASE_ANON_KEY` | `ag-session` cookie   | `local-dev.test.ts`, `roles.test.ts`                      | `auth-actions.test.ts`                        | `auth-session-shape.test.ts` (4)    | skip — `/auth/v1/health`             |

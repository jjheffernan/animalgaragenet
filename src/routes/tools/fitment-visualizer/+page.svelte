<script lang="ts">
	...
	const fitmentLabel = $derived(
		offset < -5
			? 'Poke (extends outward)'
			: offset > 5
				? 'Tuck (recessed inward)'
				: 'Flush / neutral'
	);
</script>
...
<section class="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
	<div class="grid gap-12 lg:grid-cols-2">
		<AnimatedReveal>
			...
			<div class="mt-6 rounded-sm border border-zinc-800 bg-zinc-900/50 p-6 text-sm">
				<p class="text-zinc-400">
					Estimated lip poke: <strong class="text-white">{pokeIn.toFixed(2)} in</strong>
				</p>
				<p class="mt-2 text-xs text-zinc-500">
					Positive offset tucks the wheel inward; negative offset pushes it outward. Compare against
					your fender clearance before ordering.
				</p>
			</div>
		</AnimatedReveal>
		...
		<AnimatedReveal delay={150}>
			<div
				class="relative flex h-72 items-center justify-center rounded-sm border border-zinc-800 bg-zinc-900"
			>
				<p
					class="absolute left-4 top-4 text-[10px] font-bold uppercase tracking-widest text-zinc-600"
				>
					Fender line
				</p>
				<div
					class="absolute left-1/2 top-8 h-48 w-px -translate-x-1/2 bg-zinc-600"
					aria-hidden="true"
				></div>
				<div
					class="absolute h-48 w-48 rounded-full border-4 border-zinc-700 bg-zinc-800"
					aria-hidden="true"
				></div>
				<div
				...
				></div>
				<div
					class="absolute bottom-4 left-0 right-0 flex justify-between px-6 text-[10px] font-bold uppercase tracking-widest text-zinc-500"
				>
					<span>← Poke</span>
	...
	<div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
		<AnimatedReveal>
			...
			<ul class="mt-6 space-y-4 text-sm leading-relaxed text-zinc-400">
				<li>
					<strong class="text-zinc-300">Measure twice.</strong> Check inner fender clearance with the
					suspension compressed — especially on lowered cars.
				</li>
				<li>
					<strong class="text-zinc-300">Match tire to rim.</strong> Stretching a narrow tire on a wide
					rim changes the effective contact patch and sidewall profile.
				</li>
				<li>
					<strong class="text-zinc-300">Spacers change effective offset.</strong> A 15 mm spacer reduces
					offset by 15 mm — factor that in when comparing wheels.
				</li>
				<li>
					<strong class="text-zinc-300">Hub-centric matters.</strong> Always use hub-centric rings or
					OEM-fit wheels to avoid vibration at speed.
				</li>
				<li>
					<strong class="text-zinc-300">When in doubt, ask.</strong> Our parts team can confirm fitment
					for your year/make/model before you commit.
				</li>
			</ul>
		</AnimatedReveal>
	</div>

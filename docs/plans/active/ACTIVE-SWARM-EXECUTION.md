...
## Slot counts (code-eligible, unblocked)

| Batch | Scope | Count |
|-------|--------|------:|
| **A** | API / webhooks / SEO / CI | **7** |
| **B** | Admin UI | **9** |
| **C** | Inspiration-polish code | **7** |
| **D** | Ponytail / schema / docs-only | **4** |
| | **Total code-eligible (open/partial)** | **27** |
| | **Done (ponytail + landed rows)** | **8** |
| | **Ops-blocked (no code)** | **19** |
| | **Grand total rows** | **53** |
...
| next-steps | IP-004-code | Saleor stock webhook → `restock_alerts` notify handler (signup API live; webhook stub only) | No | A | open |
...
rg '^\| [a-z]' docs/plans/active/ACTIVE-SWARM-EXECUTION.md | wc -l

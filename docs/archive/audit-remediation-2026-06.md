> **Status: Archived** — All AUD rows closed or ops-blocked. See [BLOCKERS.md](../BLOCKERS.md) · [plans/AUDIT-REMEDIATION.md](../plans/AUDIT-REMEDIATION.md) (pointer).
...
| AUD-P2-011 | Ghost Content API cache headers                 | ghost-audit               | **done** | code       | `setHeaders` / `Cache-Control` on content routes                                   | `src/lib/server/ghost/cache-headers.ts` · guides/blog loaders · BATCH-015 |
| AUD-P2-012 | Site-wide SEO / OG / analytics                  | site-audit, STATUS        | **done** | code    | Consent-gated `trackPageView`; optional `PUBLIC_ANALYTICS_SCRIPT_URL` env hook       | `src/lib/analytics.ts` · `src/lib/analytics/config.ts` · `+layout.svelte` |
| AUD-P2-013 | `prefers-reduced-motion` support                | site-audit                | **done** | code       | Animations respect user preference per style guide                                 | `AnimatedReveal.svelte` · `layout.css` · BATCH-017                        |
...
| AUD-P2-021 | daisyUI skill tree removal (post sign-off)      | agents-skills-audit       | **done** | docs       | Tree deleted; see `docs/archive/audits/zinc-only-deps-audit.md`                            | `skills-lock.json`                                                      |

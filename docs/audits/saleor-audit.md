...
Remaining gaps block production checkout: **Payment App on channel** (ops), **collection product edges**, and **catalog search at scale**. Checkout shipping, payment proxies, Stripe Elements scaffold, cart mutations, and listing quick-add are wired.
...
1. **Checkout completion & payment** — Shipping, payment proxies, Stripe Elements, and `checkoutComplete` **wired**; live pay pending Payment App on channel (ops).
2. **Cart line mutations** — **Done** — `checkoutLinesUpdate` / `checkoutLinesDelete` + `/cart/checkout` routes.
3. **Add-to-cart from listing cards** — **Done** — `ProductCard` passes default variant `variantId`.
...
- **Listing cards** (`ProductCard`) pass default Saleor `variantId` — add-line works without mock catalog lookup.
...
Read-only cart display, add-line, remove, qty update, and checkout payment proxies are wired when `PUBLIC_SALEOR_API_URL` is set. Live pay requires Payment App on channel (ops).
